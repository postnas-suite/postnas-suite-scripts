...
--Bereinigung RP - Multipolygone zu Polygonen da fuer INSPIRE nicht erlaubt

DROP TABLE IF EXISTS temp0;
SELECT gml_id, (ST_Dump( wkb_geometry )).geom as wkb_geometry INTO temp0 FROM ax_strassenverkehr WHERE ST_GeometryType( wkb_geometry ) = 'ST_MultiPolygon';
DELETE FROM temp0 WHERE ST_Area( wkb_geometry ) <= 0.2;
UPDATE ax_strassenverkehr SET wkb_geometry = (temp0.wkb_geometry) FROM temp0 WHERE ax_strassenverkehr.gml_id = temp0.gml_id;


INSERT INTO temp0 SELECT gml_id, (ST_Dump( wkb_geometry )).geom as wkb_geometry FROM ax_weg WHERE ST_GeometryType( wkb_geometry ) = 'ST_MultiPolygon';
DELETE FROM temp0 WHERE ST_Area( wkb_geometry ) <= 0.5;
UPDATE ax_weg SET wkb_geometry = (temp0.wkb_geometry) FROM temp0 WHERE ax_weg.gml_id = temp0.gml_id;
DROP TABLE IF EXISTS temp0;