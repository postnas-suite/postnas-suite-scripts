...
WHERE artderfestlegung != 8000 AND endet IS NULL AND beginnt>lastrun;
	...
	SELECT
		...
		WHEN o.gml_id LIKE 'DERP%' THEN
			coalesce(
				t.schriftinhalt,
				CASE
				WHEN artderfestlegung=4810 THEN 'SfFP'
				WHEN artderfestlegung=7000 THEN 'Weinlage'
				WHEN artderfestlegung=7100 THEN 'Weinbausteillage'
				WHEN artderfestlegung=7300 THEN 'Weinbausteilstlage'
				END
			)
